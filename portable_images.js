...
async function fetchSet(manifest,onProgress=()=>{}){if(!manifest||!Array.isArray(manifest.card_nos))throw new Error("画像対象リストがありません");if(manifest.card_nos.length>120)throw new Error("画像対象が上限を超えています");await clearOldStaging();const previousId=await active(),previous=previousId?await caches.open(cacheName(previousId)):null,id=crypto.randomUUID(),cache=await caches.open(cacheName(id)),failed=[];let done=0;for(const no of manifest.card_nos){const src=url(no);let reused=false;try{const cached=previous&&await previous.match(src);if(cached){await cache.put(src,cached.clone());reused=true}else{const response=await fetchImage(src);if(response.type!=="opaque"&&!response.ok)throw new Error("HTTP "+response.status);if(!await imageLoads(src))throw new Error("画像として開けません");await cache.put(src,response)}}catch(error){failed.push({card_no:no,error:error?.name==="AbortError"?"通信がタイムアウトしました":String(error?.message||error)})}done++;onProgress({done,total:manifest.card_nos.length,failed:failed.length});if(!reused&&done<manifest.card_nos.length)await sleep(WAIT_MS)}const downloaded=manifest.card_nos.length-failed.length;if(!downloaded){await discard(id);throw new Error("画像を取得できませんでした。通信状態を確認して再試行してください")};return{id,downloaded,total:manifest.card_nos.length,failed,card_nos:[...manifest.card_nos]}}
